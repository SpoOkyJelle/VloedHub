...
var server = http.createServer(function(req, res) {
  ...
  if (req.method === "POST" && req.url === "/api/temperature") {
    ...
    req.on("end", function() {
      ...
      logTemperatureReadings(dataT, function(err) {
        if (err) {
          res.writeHead(400, { "Content-Type": "application/json" });
  ...
  if (req.method === "GET" && req.url.indexOf("/api/temperature/history") === 0) {
    ...
    if (trange === "week") {
      tfmt = "'%Y-%m-%d'"; twhere = "received_at >= ?"; tparam = effectiveCutoff(604800000);
    } else if (trange === "month") {
      tfmt = "'%Y-%m-%d'"; twhere = "received_at >= ?"; tparam = effectiveCutoff(2592000000);
    } else {
      tfmt = "'%Y-%m-%d %H:00'"; twhere = "date(received_at) = ?"; tparam = todayAms();
    }
  ...
  if (req.method === 'GET' && req.url.indexOf('/api/led/state') === 0) {
    ...
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify(getLedState(ledDevice)));
    return;
  }
  ...
  if (req.method === 'POST' && req.url.indexOf('/api/led/state') === 0) {
    ...
    var ledStateP = getLedState(ledDeviceP);
    var bodyLed = '';
