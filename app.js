...
var ledState = { on: true, effect: 0, brightness: 180, color: { r: 255, g: 255, b: 255 } };
...
var server = http.createServer(function(req, res) {
  ...
  if (req.method === 'GET' && req.url === '/api/led/state') {
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify(ledState));
    return;
  }

  if (req.method === 'POST' && req.url === '/api/led/state') {
    var bodyLed = '';
    req.on('data', function(c) { bodyLed += c; });
    req.on('end', function() {
      try {
        var d = JSON.parse(bodyLed);
        if (d.on !== undefined) ledState.on = !!d.on;
        if (d.effect !== undefined) ledState.effect = parseInt(d.effect);
        if (d.brightness !== undefined) ledState.brightness = parseInt(d.brightness);
        if (d.color && typeof d.color === 'object') {
          var clamp255 = function(v) { v = parseInt(v); return isNaN(v) ? 0 : Math.max(0, Math.min(255, v)); };
          ledState.color = { r: clamp255(d.color.r), g: clamp255(d.color.g), b: clamp255(d.color.b) };
        }
      } catch(e) {}
      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify(ledState));
